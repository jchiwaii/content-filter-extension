...
async function exportSettings() {
  ...
  const exportData = {
    ...
    legacyType: 'wash-my-eyes-settings',
    version: '2.0',
  ...
  a.href = url;
  a.download = `safebrowse-settings-${new Date().toISOString().split('T')[0]}.json`;
  a.click();
...
function handleImportFile(e) {
  ...
  reader.onload = (event) => {
    try {
      ...
      if (!['safe-browse-settings', 'wash-my-eyes-settings'].includes(importData.type)) {
